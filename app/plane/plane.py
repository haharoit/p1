...
class Player(pygame.sprite.Sprite):
    def __init__(self, plane_img, player_rect, init_pos):
        pygame.sprite.Sprite.__init__(self)
        self.img = []  # 用来存储玩家飞机图片的列表
        for i in range(len(player_rect)):
            self.image.append(plane_img.subsurface(player_rect[i]).convert_alpha())
            self.rect = player_rect[0]  # 初始化图片所在的图形
            self.rect.topleft = init_pos  # 初始化矩形的左上角坐标
            self.speed = 8  # 初始化玩家飞机速度
            self.bullets = pygame.sprite.Group()  # 玩家所发射的子弹集合
    ...
    # 设置游戏界面大小，背景图片及标题
    # 游戏界面像素大小
    screen = pygame.display.ser_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
    ...
    text_rect.centerx = screen.get_rect().centerx + 24
